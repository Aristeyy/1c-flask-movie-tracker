...
```mermaid
flowchart LR
    A["1С:Предприятие<br/>конфигурация «КиноДашборд»<br/><br/>• Справочник Фильмы<br/>• Документ Просмотр фильма"]
    B["Flask (app.py)<br/>REST API + UI"]
    C[("SQLite БД")]
    D["OMDB API"]
    E["🌐 Веб-дашборд<br/>статистика · графики · поиск"]

    A -- "POST /api/viewing<br/>(при проведении документа)" --> B
    B --> C
    B -- "/api/fill" --> D
    D -- "постер, рейтинг, трейлер<br/>(с кэшированием)" --> B
    B --> E
```
